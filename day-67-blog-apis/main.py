...
def edit_post(post_id):
    ...
    form_edit = PostForm(obj=post_to_update)
    edit=True
    if form_edit.validate_on_submit():
        form_edit.populate_obj(post_to_update)
        post_to_update.date = f"Edited {date.today().strftime('%B %d, %Y')}"
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("make-post.html", form=form_edit, edit=edit)
